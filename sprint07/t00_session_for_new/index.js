...
const {updateSession, getSession, removeSession} = require("./session");
...
const listener = async (req, res) => {
    ...
    else if (newSessionData && Object.keys(newSessionData).length !== 0) updateSession(req.socket.remoteAddress, newSessionData)
    ...
    let data
    if (sessionData) {
        data = await resolveOutput('/savedSessionTemplate.html')
        data.body = data.body.replace('{{data}}', Object.entries(sessionData).reduce((acc, [k, v]) => `${acc}<b>${k}:</b> ${v}\n`, ''))
    } else
        data = await resolveOutput('/formTemplate.html')
    res.statusCode = 200
